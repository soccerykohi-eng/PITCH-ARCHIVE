...
export default function AdminOperations({ users,onNotice }:{ users:User[];onNotice:(message:string)=>void }) {
  const [announcements,setAnnouncements]=useState<Announcement[]>([]);const [logs,setLogs]=useState<Log[]>([]);
  ...
  const load=useCallback(async () => {
    const [a,l]=await Promise.all([fetch("/api/admin/announcements",{ cache:"no-store" }),fetch("/api/admin/logs",{ cache:"no-store" })]);
    if (a.ok) setAnnouncements((await a.json()).announcements ?? []);if (l.ok) setLogs((await l.json()).logs ?? []);
  },[]);
  ...
  return <section className="admin-operations"><div className="admin-library-head"><div><p className="admin-step">03 / OPERATIONS</p><h3>運営センター</h3><p>お知らせと操作履歴をまとめて管理します。</p></div></div>
    <Tabs defaultValue="announcements" className="operations-tabs"><TabsList className="operations-nav"><TabsTrigger value="announcements">お知らせ</TabsTrigger><TabsTrigger value="logs">操作ログ</TabsTrigger></TabsList>
