...
export default function SafetySettings({onBack,onNotice}:{onBack:()=>void;onNotice:(message:string)=>void}){
  ...
  return <section className="safety-settings"><header><button type="button" onClick={onBack}>‹ アカウント設定</button></header><h1>プライバシー・安全</h1><section><div><strong>ブロック中のユーザー</strong><span>{people.length}人</span></div>{people.length?<div className="blocked-user-list">{people.map((person)=><div key={person.email}><i className={person.avatarUrl?"has-photo":""} style={person.avatarUrl?{backgroundImage:`url(${person.avatarUrl})`}:undefined}>{person.avatarUrl?"":person.displayName.slice(0,1)}</i><strong>{person.displayName}</strong><Button variant="outline" disabled={busy===person.email} onClick={()=>void unblock(person.email)}>解除</Button></div>)}</div>:<p>ブロック中のユーザーはいません。</p>}</section><section><strong>フレンドIDについて</strong><p>IDを知っている人だけが、あなたへフレンド申請できます。参加者一覧は公開されません。</p></section></section>
}
