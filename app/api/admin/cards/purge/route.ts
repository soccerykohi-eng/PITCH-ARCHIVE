...
async function impact(id:string):Promise<PurgeImpact> {
  const row=await getRawDb().prepare(`SELECT
    ...
    .bind(id,id,id,id,id,id,id,id,id).first<PurgeImpact>();
  return { owners:Number(row?.owners ?? 0),copies:Number(row?.copies ?? 0),packs:Number(row?.packs ?? 0),openingsPreserved:Number(row?.openingsPreserved ?? 0),claimsPreserved:Number(row?.claimsPreserved ?? 0),trades:Number(row?.trades ?? 0),exchangeOffers:Number(row?.exchangeOffers ?? 0),activePacks:Number(row?.activePacks ?? 0) };
}
...
export async function POST(request:Request) {
  ...
  const db=getRawDb();
  const detail=JSON.stringify({ cardName:card.name,owners:currentImpact.owners,copies:currentImpact.copies,packs:currentImpact.packs,trades:currentImpact.trades,openingsPreserved:currentImpact.openingsPreserved,claimsPreserved:currentImpact.claimsPreserved });
  await db.batch([
