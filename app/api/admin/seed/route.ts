...
export async function POST() {
  ...
  BUILTIN_CARDS.forEach((card,index) => {
    statements.push(db.prepare(`INSERT OR IGNORE INTO cards (id,name,position,country,team,number,rating,rarity,series,card_type,season,image_key,created_at) VALUES (?,?,?,?,?,NULL,80,?,?,'LEGACY','—',?,?)`).bind(card.id,card.name,card.position,card.country,card.team,card.rarity,card.series,card.imageKey,Date.now()));
