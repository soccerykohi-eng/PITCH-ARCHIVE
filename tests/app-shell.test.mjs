...
test("uses the mobile app shell at supported phone widths", () => {
  ...
  assert.match(css, /height:calc\(68px \+ env\(safe-area-inset-bottom\)\)/);
  assert.match(css, /padding:6px 8px env\(safe-area-inset-bottom\)/);
  assert.match(css, /padding:calc\(env\(safe-area-inset-top\) \+ var\(--pa-space-2\)\)/);
  assert.match(css, /padding:0 var\(--pa-space-4\) calc\(88px \+ env\(safe-area-inset-bottom\)\)/);
  assert.match(css, /\.brand-lockup h1 \{ display:block;/);
  assert.match(css, /\.collection-pill,\.account-button \{ display:none!important;/);
  assert.match(css, /\.network-nav button \{ width:100%;height:56px!important;min-height:56px;/);
});
